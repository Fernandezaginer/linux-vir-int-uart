...
namespace nerfnet
{
  namespace
  {
    ...
    TEST(RadioTransportReceiverFrameTest, BuildSubFramesSingle)
    {
      ...
      EXPECT_EQ(DecodeU32(
                    static_cast<std::string_view>(sub_frames[0]).substr(0)),
                16);
      EXPECT_EQ(DecodeU32(
                    static_cast<std::string_view>(sub_frames[0]).substr(4)),
                0);
      EXPECT_EQ(DecodeU32(
                    static_cast<std::string_view>(sub_frames[0]).substr(8)),
                16);
    ...
    TEST(RadioTransportReceiverFrameTest, BuildSubFramesMulti)
    {
      ...
      EXPECT_EQ(DecodeU32(
                    static_cast<std::string_view>(sub_frames[1]).substr(0)),
                1004);
      EXPECT_EQ(DecodeU32(
                    static_cast<std::string_view>(sub_frames[1]).substr(4)),
                7188);
      EXPECT_EQ(DecodeU32(
                    static_cast<std::string_view>(sub_frames[1]).substr(8)),
                8192);
