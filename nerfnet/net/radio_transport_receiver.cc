...
namespace nerfnet
{
                                          ...
                                          size_t max_sub_frame_size)
  {
    ...
    const size_t max_sub_frame_payload_length = max_sub_frame_size - kPayloadHeaderSize;
      ...
      const Link::Frame &frame)
  {
    ...
    if (receive_state_.has_value() && receive_state_->address == frame.address)
    {
      ...
      else if (frame_type == FrameType::PAYLOAD)
      {
        ...
        if (receive_state_->pieces.find(sequence_id) == receive_state_->pieces.end())
        {
    ...
    else if (last_receive_state_.has_value() && last_receive_state_->address == frame.address)
    {
    ...
    else if (!receive_state_.has_value() && frame_type == FrameType::BEGIN && !frame_ack)
    {
  ...
  void RadioTransportReceiver::HandleTimeout()
  {
    if (receive_state_.has_value())
    {
      if ((clock_->TimeNowUs() - receive_state_->receive_time_us) > kReceiverTimeoutUs)
      {
    ...
    {
      if ((clock_->TimeNowUs() - last_receive_state_->receive_time_us) > kReceiverTimeoutUs)
      {
