...
namespace nerfnet
{
  ...
  void RadioTransport::BeaconThread()
  {
    while (transport_running_)
    {
      ...
      if ((time_now_us - last_beacon_time_us_) > (config_.beacon_interval_us + beacon_jitter_us))
      {
        std::unique_lock<std::mutex> lock(link_mutex_);
      ...
      uint64_t next_beacon_time_us = last_beacon_time_us_ + config_.beacon_interval_us;
    ...
    while (receive_result != Link::ReceiveResult::SUCCESS)
    {
      ...
      while (receive_result != Link::ReceiveResult::SUCCESS)
      {
        ...
        if (receive_result == Link::ReceiveResult::SUCCESS)
        {
          ...
          else if ((frame.payload[0] & kMaskFrameType) != static_cast<uint8_t>(frame_type))
          {
      ...
      if (receive_result == Link::ReceiveResult::SUCCESS && out_frame != nullptr)
      {
        *out_frame = frame;
      }
    }
